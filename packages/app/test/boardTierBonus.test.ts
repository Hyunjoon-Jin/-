...
describe('시즌 요약: 이사회 신뢰 등급 상승 투자 보너스 (C-new1)', () => {
  it('여러 시즌 진행 중 신뢰 등급이 오르면 boardTierBonus가 실리고 자금이 늘어난다', () => {
    ...
    // 검증 목적에 맞는 시드로 교체.
    // 시드 2028은 고도화 항목46(심판 엄격도 도입으로 카드·페어플레이·이사회 신뢰
    // 궤적이 바뀜)으로 다시 20시즌 내내 등급 상승을 볼 수 없는 경로가 됐다 — 2033으로 재교체.
    // 시드 2033은 고도화 항목56(세컨드 옐로우 → 레드 자동 전환으로 카드·경기 결과
    // 궤적이 다시 바뀜)으로 또 등급 상승을 볼 수 없는 경로가 됐다 — 2020으로 재교체.
    let g = startGame(2020, 'c0');
    let sawBonus = false;
